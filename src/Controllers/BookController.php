...
class BookController extends AbstractController
{
    ...
    public function upload(Request $request, FileUploader $fileUploader, BookService $bookService): JsonResponse
    {
        ...
        $requiredFields = ['title', 'author', 'isbn', 'publication_year', 'edition', 'translator', 'description', 'category_id'];
...
public function getAllFiles(Request $request, BookService $bookService): JsonResponse
    {
    try {
        $nbOnPage = (int) $request->query->get('NB_ON_PAGE', 10);
        $categoryParam = $request->query->get('CATEGORY', 'ALL');
        $order = strtoupper($request->query->get('ORDER', 'ASC'));
        if (!in_array($order, ['ASC', 'DESC'])) {
            $order = 'ASC';
        }

        // Si category est 'ALL', on passe null, sinon on cast en int
        $category = ($categoryParam === 'ALL') ? null : (int) $categoryParam;

        $books = $bookService->getAllFiles($category, $nbOnPage, $order);
        ...
        foreach ($books as $book) {
            $booksData[] = [
                ...
                'views' => $book->getViews(),
                'category' => $book->getCategory() ? $book->getCategory()->getLabel() : null,
                'link' => $book->getLink()
            ];
        }
